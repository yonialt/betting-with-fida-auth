...
import { oddsService } from './src/server/oddsService';
...
// --- Odds API & Public Odds Endpoints ---
app.get('/api/odds/status', async (_req, res) => {
  const oddsStatus = oddsService.getStatus();
  const fbStatus = apiFootballService.getStatus();
  const cacheStats = await redisCache.getStats();
  res.json({
    odds: oddsStatus,
...
app.post('/api/odds/configure', (req, res) => {
  const { apiKey, provider } = req.body || {};
  if (apiKey !== undefined) {
    oddsService.setApiKey(apiKey, provider || 'odds-api');
  }
  res.json({
    success: true,
...
app.get('/api/odds/live', async (req, res) => {
  ...
  try {
    const odds = await oddsService.getLiveOdds(sport);
    res.json(odds);
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
...
app.get('/api/odds/upcoming', async (req, res) => {
  const sport = (req.query.sport as string) || 'football';
  try {
    const odds = await oddsService.getUpcomingOdds(sport);
    res.json(odds);
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
...
app.get('/api/odds/match/:matchId', async (req, res) => {
  try {
    const markets = await oddsService.getMatchOdds(req.params.matchId);
    res.json(markets);
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
...
app.post('/api/odds/sync', async (_req, res) => {
  try {
    const result = await oddsService.syncAllFromApi();
    const cacheStats = await redisCache.getStats();
    res.json({
      success: true,
      ...result,
...
app.get('/api/odds/prediction', async (req, res) => {
  const category = (req.query.category as string) || 'sports';
  try {
    const markets = await oddsService.getPredictionMarkets(category);
    res.json({
      markets,
...
app.get('/api/odds/prediction/:marketId', async (req, res) => {
  const category = (req.query.category as string) || 'sports';
  try {
    const markets = await oddsService.getPredictionMarkets(category);
    const market = markets.find((m: any) => m.id === req.params.marketId);
    if (!market) {
...
// --- Odds Comparison & Multi-Source Aggregation ---
app.get('/api/odds/compare', async (req, res) => {
  const sport = (req.query.sport as string) || 'football';
  try {
    const [oddsApi, fbOdds] = await Promise.all([
      oddsService.getLiveOdds(sport),
      apiFootballService.getLiveMatches(sport),
    ]);
    res.json({
      oddsApi,
      footballMatches: fbOdds,
      comparison: {
        totalOddsSources: 2,
        bestOddsCount: oddsApi.filter((m) => m.odds.w1 && m.odds.w1.value < 2).length,
      },
    });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
