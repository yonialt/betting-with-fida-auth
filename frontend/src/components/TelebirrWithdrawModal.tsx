...
export const TelebirrWithdrawModal: React.FC = () => {
  ...
  const handleContinue = () => {
    if (amount < 100) {
      setNotification({ message: 'Minimum withdraw is ETB 100', type: 'warning' });
      return;
    }
    if (amount > balance) {
  ...
  const handlePinSubmit = useCallback(async () => {
    ...
    // Complete the withdrawal via the wallet context (validates balance,
    // deducts client-side, and best-effort notifies the backend).
    try {
      const fullPhone = phone.startsWith('+251') ? phone : '+251' + phone.replace(/^0/, '');
      const ok = withdrawFunds(amount, fullPhone);
      if (!ok) {
        setErrorMsg('Withdrawal could not be completed. Please check your balance and try again.');
        setStep('error');
        return;
      }
      setReceiptRef('WD' + Date.now());
      setStep('success');
      setNotification({ message: `Withdrawal of ${fmt(amount)} completed!`, type: 'success' });
    } catch {
      setStep('error');
      setErrorMsg('Withdrawal verification failed. Contact support if funds were deducted.');
    }
  }, [pin, phone, amount, withdrawFunds]);
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 overflow-y-auto">
        ...
        <div className="relative w-full max-w-[420px] rounded-2xl overflow-hidden shadow-3xl my-auto max-h-[94vh] overflow-y-auto" style={{ background: '#f5f5f5', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif' }}>
          ...
          <div style={{ background: 'white', margin: '16px', borderRadius: 12, padding: '40px 20px', textAlign: 'center' }}>
            ...
            <h2 style={{ fontSize: 18, fontWeight: 700, margin: '0 0 8px', color: '#1a1a1a' }}>Processing withdrawal...</h2>
            <p style={{ fontSize: 14, color: '#888' }}>Please wait while we confirm your withdrawal</p>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 overflow-y-auto">
        ...
        <div className="relative w-full max-w-[420px] rounded-2xl overflow-hidden shadow-3xl my-auto max-h-[94vh] overflow-y-auto" style={{ background: '#f5f5f5', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif' }}>
          ...
          <div style={{ background: 'white', margin: '16px', borderRadius: 12, padding: '30px 20px', textAlign: 'center' }}>
            ...
            <h2 style={{ fontSize: 20, fontWeight: 700, margin: '0 0 8px', color: '#2e7d32' }}>Withdrawal Successful</h2>
            <p style={{ fontSize: 14, color: '#666', margin: '0 0 4px' }}>You have withdrawn <strong>{fmtFull(amount)}</strong></p>
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1fr_1.15fr] overflow-y-auto flex-1 min-h-0 divide-y md:divide-y-0 md:divide-x divide-[#DFE5F0]">
          <div className="p-4 sm:p-6 flex flex-col justify-between">
            <div>
              <h1 className="text-xl font-semibold mb-1" style={{ fontFamily: "'Space Grotesk', sans-serif", color: '#10213D' }}>Withdraw from your wallet</h1>
              <p className="text-sm mb-5" style={{ color: '#4C5C77' }}>Current balance · {balance.toLocaleString()} {currency}</p>
              ...
              <div className="mb-4">
                ...
                <p className="text-xs mt-1.5" style={{ color: '#4C5C77' }}>Minimum withdraw ETB 100 · maximum ETB 50,000 per transaction.</p>
              </div>
            ...
            <div className="pt-3 border-t border-[#DFE5F0]">
              <div className="flex justify-between text-sm py-0.5" style={{ color: '#4C5C77' }}><span>Withdraw amount</span><span>{fmt(amount)}</span></div>
              ...
              <div className="flex justify-between font-semibold pt-2 mt-1 border-t border-[#DFE5F0]" style={{ color: '#10213D' }}>
                <span className="text-base">Total to withdraw</span>
              ...
              <div className="bg-white border border-[#DFE5F0] rounded-[14px] p-4 sm:p-5 flex flex-col">
                {step === 'error' && (
                  <div className="flex flex-col items-center text-center flex-1 justify-center gap-3 py-3">
                    ...
                    <h3 className="m-0 mb-1 text-lg" style={{ fontFamily: "'Space Grotesk', sans-serif", color: '#10213D' }}>Withdraw failed</h3>
                ...
                {(step === 'form' || step === 'processing') && (
                  <>
                    <div className="flex items-center justify-between mb-3">
                      <span className="font-semibold text-base" style={{ fontFamily: "'Space Grotesk', sans-serif", color: '#10213D' }}>Withdraw with telebirr</span>
                    </div>
                    <p className="text-xs sm:text-sm leading-relaxed mb-3" style={{ color: '#4C5C77' }}>
                      You'll be redirected to telebirr to approve this withdrawal. {BRAND} never sees or stores your telebirr PIN — you enter it only inside telebirr's own app or USSD session.
                    </p>
                    <div className="flex gap-2.5 items-start rounded-[10px] px-3.5 py-2.5 text-xs leading-relaxed mb-4 bg-[#E4EEFC] border border-[#DFE5F0] text-[#083761]">
                      ...
                      <span>After you tap continue, you'll see the telebirr checkout: phone number, full name, then your telebirr PIN to authorize the withdrawal.</span>
                    </div>
