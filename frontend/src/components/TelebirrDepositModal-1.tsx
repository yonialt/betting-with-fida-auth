...
export const TelebirrDepositModal: React.FC = () => {
  const { depositModalOpen, setDepositModalOpen, user, setNotification, depositFunds } = useBetting();
  ...
  // Reset when the modal opens. Depend ONLY on depositModalOpen — depending on
  // `user` would re-fire this when depositFunds updates the balance and bounce
  ...
  const handleContinue = () => {
    ...
    if (amount > 50000) {
      setNotification({ message: 'Maximum deposit is ETB 50,000 per transaction', type: 'warning' });
      return;
    }
          ...
          <div className="p-4 sm:p-6 flex flex-col justify-between">
            <div>
              ...
              <p className="text-sm mb-5" style={{ color: '#4C5C77' }}>
                Current balance · {user?.balance?.toLocaleString() || '0'} {user?.currency || 'ETB'}
              </p>
          ...
          <div className="p-4 sm:p-6 flex flex-col justify-between min-h-0">
            <div>
              ...
              <div className="bg-white border border-[#DFE5F0] rounded-[14px] p-4 sm:p-5 flex flex-col">
                ...
                <p className="text-xs sm:text-sm leading-relaxed mb-3" style={{ color: '#4C5C77' }}>
                  You'll be redirected to telebirr to approve this payment. {MERCHANT} never sees or stores your telebirr
                  password — you enter it only inside telebirr's own secure screen.
                </p>
                <div className="flex gap-2.5 items-start rounded-[10px] px-3.5 py-2.5 text-xs leading-relaxed mb-4 bg-[#E4EEFC] border border-[#DFE5F0] text-[#083761]">
                  ...
                  <span>
                    ...
                    confirm the payment — all inside telebirr.
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="pt-3 mt-auto">
              <button
                ...
                className="w-full border-none rounded-[10px] px-4 py-3 text-sm font-semibold text-white transition-colors cursor-pointer shadow-xs hover:brightness-105"
