...
/** Tailwind's `lg` breakpoint: below it the slip is a bottom sheet, above it a column. */
const DESKTOP_SLIP_QUERY = '(min-width: 1024px)';

/** Tracks a CSS media query so we can render the desktop column or the mobile sheet. */
...
export const BetSlip: React.FC = () => {
  ...
  // On phones/tablets the slip lives in a bottom sheet instead of the right column.
  const [isMobileSlipOpen, setIsMobileSlipOpen] = React.useState<boolean>(false);
  const isDesktop = useMediaQuery(DESKTOP_SLIP_QUERY);
  const isSheetOpen = isMobileSlipOpen && !isDesktop;

  const handleQuickAddStake = (amount: number) => {
    setStakeAmount((prev) => Math.max(1, +(prev + amount).toFixed(2)));
  };

  const openMobileSlip = (tab: 'slip' | 'mybets') => {
    setActiveTabSlip(tab);
  ...
  const tabsBar = (
    <div className="flex items-center border-b border-neutral-200 bg-[#fbfcfd] shrink-0">
      <button
        id="tab-bet-slip"
  ...
  const slipTabBody = (
    <div className="flex-1 flex flex-col justify-between p-3 lg:ml-2 overflow-y-auto">
              ...
              >
                ...
                <div className="flex items-center justify-between text-[11px] mb-1">
                  <div className="flex items-center gap-1.5 min-w-0">
                    {item.isLive && (
                      <span className="bg-[#ff4d4f] text-white text-[9px] font-black px-1.5 py-0.2 rounded-xs uppercase tracking-wider shrink-0">
                        LIVE
                      </span>
                    )}
                ...
                <div className="flex items-center justify-between mt-1.5 pt-1.5 border-t border-neutral-200/60 gap-2">
                  <div className="flex items-center gap-1.5 min-w-0">
                    <span className="bg-[#ffc600] text-black font-extrabold text-xs px-1.5 py-0.5 rounded-xs font-mono shrink-0">
                      {item.odds}
                    </span>
                  ...
                  <span className="text-[11px] text-neutral-500 truncate max-w-[100px] shrink-0">
                    {item.selectionName}
                  </span>
                </div>
              </div>
        ...
        <div>
          ...
          <div className="flex items-center rounded border border-neutral-300 overflow-hidden bg-white">
            ...
            <input
              ...
              className="flex-1 min-w-0 text-center font-mono font-bold text-sm text-neutral-900 py-1 focus:outline-hidden"
            />
            <button
  ...
  const myBetsTabBody = (
    <div className="flex-1 p-3 lg:ml-2 overflow-y-auto space-y-3">
  ...
  const panelBody = activeTabSlip === 'slip' ? slipTabBody : myBetsTabBody;

  /* ------------------------------------------------------------------ *
   * Desktop / large screens: the classic third column of the page row.
   * ------------------------------------------------------------------ */
  if (isDesktop) {
    ...
    return (
      <aside
        id="bet-slip-panel"
        className="w-80 xl:w-96 bg-white border-l border-neutral-200 flex flex-col shrink-0 select-none shadow-xs h-full min-h-[500px]"
        style={{
          marginLeft: '2px',
          paddingTop: '0px',
          paddingBottom: '5px',
      ...
      >
        ...
        <div className="w-full bg-[#1c2024] border-t-2 border-[#ffc600] border-b border-neutral-800 shrink-0">
          <button
  ...
  /* ------------------------------------------------------------------ *
   * Phones & tablets: the match list owns the full width, the slip is a
   * bottom sheet opened from a fixed bottom bar. Rendered through a
   * portal so the page containers can never clip or offset it.
   * ------------------------------------------------------------------ */
  const mobileUI = (
    <>
      <div
        id="mobile-betslip-bar"
        className="fixed bottom-0 inset-x-0 z-40 bg-[#1b2838] border-t-2 border-[#ffc600] flex items-stretch text-white shadow-[0_-2px_10px_rgba(0,0,0,0.25)]"
        style={{ paddingBottom: 'env(safe-area-inset-bottom)' }}
      >
        <button
          id="btn-mobile-open-slip"
          onClick={() => openMobileSlip('slip')}
          className="flex-1 h-[50px] flex items-center justify-center gap-2 text-sm font-bold active:bg-[#233449] transition-colors cursor-pointer"
        >
          <span>Bet slip</span>
          {betSlip.length > 0 && (
            <span className="min-w-4 h-4 px-1 rounded-full bg-[#ffc600] text-black text-[10px] font-black flex items-center justify-center">
              {betSlip.length}
            </span>
          )}
        </button>

        <span className="w-px bg-[#33465c] my-2" aria-hidden="true" />

        <button
          id="btn-mobile-open-mybets"
          onClick={() => openMobileSlip('mybets')}
          className="flex-1 h-[50px] flex items-center justify-center gap-2 text-sm font-bold active:bg-[#233449] transition-colors cursor-pointer"
        >
          <span>My bets</span>
          {activeBetsCount > 0 && (
            <span className="min-w-4 h-4 px-1 rounded-full bg-neutral-600 text-white text-[10px] font-black flex items-center justify-center">
              {activeBetsCount}
            </span>
          )}
        </button>
      ...
      <aside
        ...
        aria-label={activeTabSlip === 'slip' ? 'Bet slip' : 'My bets'}
        className={`fixed inset-x-0 bottom-0 z-50 bg-white border-t-2 border-[#ffc600] rounded-t-2xl shadow-2xl flex flex-col max-h-[85dvh] transition-transform duration-300 ease-out ${
      ...
      >
        ...
        <div className="shrink-0 border-b border-neutral-200">
          ...
          <div className="flex items-center justify-between pl-3 pr-2 pb-1.5">
            <span className="text-sm font-extrabold uppercase tracking-tight text-neutral-800">
              {activeTabSlip === 'slip' ? 'Bet slip' : 'My bets'}
            </span>
            <button
              id="btn-close-mobile-betslip"
              onClick={() => setIsMobileSlipOpen(false)}
              className="w-9 h-9 flex items-center justify-center rounded-md text-neutral-500 hover:text-black hover:bg-neutral-100 active:scale-95 transition-all cursor-pointer"
              title="Close"
            >
              <X className="w-5 h-5" />
            </button>
